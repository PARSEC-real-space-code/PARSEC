  ...
  integer
ioffset,itot,icur,icount,irow,itran,wsirow,wsitran,wsicur,wsitot,wsicount
  ...
  !
  !  Initialize the atom counter, ja, and arrays in nloc_p_pot
  !  structure.
  ja = 0
  ...
  do ity = 1, clust%type_num
     ...
     do iat = 1, clust%natmi(ity)
        ...
        za = clust%zatm(ja)
        !
        !  Check if the grid point is within the core-cutoff radius
        !  around
        !  the atom. If it is not, it has no non-local contribution
        !  because
        ...
        !  +x,-x,+y,-y,+z,-z directions (and any combination thereof).
        !  In
        !  theory, one would need to check for shifts of twice the size
        !  of
        !  the box, three times the size of the box, etc. In practice
        !  this
...
!$OMP& DEFAULT(SHARED) &
!$OMP&
!PRIVATE(mg,icellx,icelly,icellz,uvec,x,y,z,xvec,rr_tmp,lnloc_count,wslnloc_count)
!&
!$OMP& REDUCTION(+:nloc,wsnloc)
        do mg = 1, grid%ndim
           !  Allow each point to be shifted by the length of the box in
           !  each
           !  direction.
           do icellx = -nreplica(1),nreplica(1)
              do icelly = -nreplica(2),nreplica(2)
                 do icellz = -nreplica(3),nreplica(3)
                    !  Compute distance between point (or its replica if
                    !  PBC)
                    ...
                    do idgx = -ndouble + 1, ndouble - 1
                       do idgy = -ndouble + 1, ndouble - 1
                          do idgz = -ndouble + 1, ndouble - 1
                            !no need to keep looking if this is a nonloc
                            !point
                            if((wslnloc_count == 0 ) .AND. ( lnloc_count
== 0)) then
                             ...
                             if (rr_tmp <= rc(ity)) lnloc_count =
lnloc_count+1
                             !if (rr_tmp <= p_pot%rws(ity)) wslnloc =
                             !.true.
                             if (rr_tmp <= p_pot%rws(ity)) wslnloc_count
= wslnloc_count+1
                            endif
                          enddo
                       enddo
...
!$OMP END PARALLEL DO
  ...
  do ity = 1, clust%type_num
     do iat = 1, clust%natmi(ity)
        ja = ja + 1
        !  Set some identifying arrays for atoms with spin-orbit
        !  coupling.
  ...
  !     the irreducible wedge) and its location on the cell,
  !     respectively.
                    ...
                    do idgx = -ndouble + 1, ndouble - 1
                       do idgy = -ndouble + 1, ndouble - 1
                          do idgz = -ndouble + 1, ndouble - 1
                          !if this point is flagged, no need to keep
                          !comparing
                            if ( (.NOT. lnloc) .AND. (.NOT. wslnloc) )
then
                            ...
                            .and. (idgz == 0) ) then
                             ...
                             if (rr_tmp <= p_pot%rws(ity)) wslnloc =
.true.
                             !if (rr_tmp <= p_pot%rws(ity))
                             !wslnloc_count = wslnloc_count +1
                            endif
                          enddo
                       enddo
                    enddo
...
/)
  ...
  write(9,*) '-----------------------------------'
  write(9,*) 'Max # of nonlocal points for one atom =
',nloc_p_pot%maxnloc
  write(9,*)
...
',nloc_p_pot%maxnloc
     ...
     if (nloc_p_pot%is_so) write(7,*) &
          'Tot. number of atoms with spin-orbit term
=',nloc_p_pot%so_num
     write(7,*)
  endif
  !
  ...
  do ity = 1, clust%type_num
     ...
     do iat = 1, clust%natmi(ity)
        ...
        do lp = 1, p_pot%nlocp(ity)
           ...
           !  (radial pseudowavefunction)*(angular wave function), put
           !  the
           !  results in anloc. That is, calculate explicitly
           !  |Vl-Vloc|phi_lm>.
...
log(one+ainv(ity)*rnorm))+1
                          ...
                          if (p_pot%uspline(ity)) then
                             call
splint(p_pot%rs(-p_pot%norder:npoint,ity), &
...
fdouble(idgx)*fdouble(idgy)*fdouble(idgz)
                       ...
                       !   Do the same for the spin-orbit potentials,
                       !   this
                       !   time with complex arrays here s orbitals have
                       !   no
                       !   contribution to spin-orbit and local
                       !   component has
                       !   a contribution.
                       if(nloc_p_pot%is_so .and. lp > 1) then
                          if (nloc_p_pot%so(ja)) then
                             ...
                             dvr_so  = p_pot%dvr_so(npt,isy,sol) +
fact1* &
                               (p_pot%dvr_so(npt+1,isy,sol)-p_pot%dvr_so(npt,isy,sol))

                             dvr_ion  = p_pot%dvr_ion(npt,isy,sol) +
fact1* &
...
dvr_ion*zylm(im)*rr
                             ...
                             enddo
                          endif !if(p_pot%spinorbit .and. ... .ne....)
then
...
zvylmd_tmpi(:,im)
  ...
  if(wsmaxnloc>0) then
  call
pisum(nloc_p_pot%wsindw,wsmaxnloc*nloc_p_pot%atom_num,nnodes,comm)
  call
pisum(nloc_p_pot%wstran,wsmaxnloc*nloc_p_pot%atom_num,nnodes,comm)
  endif
  ...
  do iso = 1, nloc_p_pot%so_num
     ...
     i = mod(ja-1,nnodes)
     call MPI_BCAST(nloc_p_pot%v_so(1,1,iso),maxnloc*8,
MPI_DOUBLE_COMPLEX,i,comm,mpinfo)
     call MPI_BCAST(nloc_p_pot%v_ion(1,1,iso),maxnloc*8,
MPI_DOUBLE_COMPLEX,i,comm,mpinfo)
     call MPI_BCAST(nloc_p_pot%cc(1,iso),2,
MPI_DOUBLE_PRECISION,i,comm,mpinfo)
     call MPI_BCAST(nloc_p_pot%dv_so(1,1,1,iso),maxnloc*24,
MPI_DOUBLE_COMPLEX,i,comm,mpinfo)
     call MPI_BCAST(nloc_p_pot%dv_ion(1,1,1,iso),maxnloc*24,
MPI_DOUBLE_COMPLEX,i,comm,mpinfo)
  enddo
#endif
#ifdef OLDDEBUG
     ...
     do i = 1, nloc
        ...
        if (parallel%irows(parallel%group_iam) <= irow.and.irow < &
           ...
           do lm = 1, nloc_p_pot%nlmatm(ja)
              ...
              nloc_p_pot%vylmd(:,icur,ilm+lm) =
nloc_p_pot%vylmd(:,i,ilm+lm)
           enddo
           do lm = 1, 8
              if(nloc_p_pot%so(ja)) then
                 nloc_p_pot%v_so(icur,lm,iso) =
nloc_p_pot%v_so(i,lm,iso)
                 nloc_p_pot%v_ion(icur,lm,iso) =
nloc_p_pot%v_ion(i,lm,iso)
                 nloc_p_pot%dv_so(:,icur,lm,iso) =
nloc_p_pot%dv_so(:,i,lm,iso)
                 nloc_p_pot%dv_ion(:,icur,lm,iso) =
nloc_p_pot%dv_ion(:,i,lm,iso)
              endif
           enddo
           if (pbc%nkpt > 0) then
...
&
  ...
  enddo
  write(9,*) 'My number of rows in WS sphere: ',wsicount,' of total
',wsitot

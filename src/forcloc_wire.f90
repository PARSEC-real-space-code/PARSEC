...
!
!---------------------------------------------------------------
subroutine forcloc_wire(clust,grid,pot,p_pot,rsymm,parallel,alatt,rho,ipr)
  ...
  !  length of periodic cell along x direction
  ...
  real(dp) :: fc, f_c(3)
  !  counters
  integer ii, ja, ity, iat, igrid, jgrid, itrans, icell
  real(dp) :: rw(3), rrw(3)
  !  allocation check
  integer alcstat
  ...
  do ity = 1, clust%type_num
     ...
     nreplica = nint( six * sigma / alatt ) + 1
...
end subroutine forcloc_wire
!===============================================================
!
!  Calculate the characters of the gradient operator by taking one
!  point in the irreducible wedge (point of order itest),
!  operating all symmetry operations on it and taking the ratio
!  between coordinates of the rotated point versus the original
!  point.
!
!  WARNING: Point rtest must have all Cartesian components different
!  from zero.
!  
!---------------------------------------------------------------
subroutine grad_char(grid,rsymm,parallel,chi_grad)
  ...
  !  symmetry operations in reduced group:
  type (symmetry), intent(in) :: rsymm
  !  characters of the gradient
  integer, intent(out) :: chi_grad(rsymm%ntrans,3)
  !
  !  Work variables:
  !
  !  order of pint in the irreducible wedge
  !  (any value 0 < itest <= nwedge is acceptable)
  integer :: itest
  !  counters
  integer :: ii, itrans
  real(dp) :: rtest(3), rr(3)
  real(dp), parameter :: small = 1.d-6

  !---------------------------------------------------------------

  do itest = 1, parallel%nwedge
    rtest(1) = (grid%shift(1) + grid%kx(itest))*grid%step(1)
    rtest(2) = (grid%shift(2) + grid%ky(itest))*grid%step(2)
    rtest(3) = (grid%shift(3) + grid%kz(itest))*grid%step(3)
    if (abs(rtest(1)).gt.small) then
      if (abs(rtest(2)).gt.small) then
        if (abs(rtest(3)).gt.small) then
          exit
        endif
      endif
    endif
  enddo
  do itrans = 1, rsymm%ntrans
     call symop(rsymm,itrans,rtest,rr)
     do ii = 1, 3
        chi_grad(itrans,ii) = nint( rr(ii) / rtest(ii) )
     enddo
  enddo

end subroutine grad_char
!===============================================================

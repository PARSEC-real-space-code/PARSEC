...
!
!---------------------------------------------------------------
subroutine ewald_sum(clust,pbc,eewald,ipr,zv,zvpt)
  ...
  real(dp) :: zz(pbc%atom_num*pbc%type_num+clust%nptchrg*clust%npttyp)
  ...
  if(clust%has_ptchrg) then
     do i = 1, clust%npttyp
        do j = 1, clust%nptt(i)
           k = k + 1
           zz(k) = zvpt(i)
           rc(1:3,k) =  pbc%ratpt(1:3,j,i)*twopio
           write(7,'(3(3x,f11.6))') pbc%ratpt(:,j,i)
           do n = 1, 3
              if (rc(n,k) >= one) &
                   rc(n,k) = modulo(rc(n,k) + eight,one)
              if (rc(n,k) < zero) &
  ...
  do i = 2, ngg
     ...
     do j = 1, clust%atom_num + clust%nptchrg
  ...
  do i = 2, clust%atom_num + clust%nptchrg

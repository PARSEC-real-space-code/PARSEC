!===============================================================
! Multisecant methods for mixing, hybrid update on approximate
! Jacobian. More precisely, at each iteration, Type-I or Type-II
! update is chosen according to the secant errors.
!
! Reference:
...
subroutine msecant3_mix(mixer,parallel,iter,nrep,xin,xout,ierr)
    ...
    real(dp) :: f(mixer%ndim * mixer%nspin)
    real(dp), allocatable :: tmp(:)
    ! Counters, temporary variables, etc.
    integer :: i, j, k, m
    integer :: sz, res, ngroup
    integer :: info, alcstat
    real(dp) :: lhs, rhs, t0, t1, t2
    real(dp), dimension(1) :: lhsvec, rhsvec, t0vec, t1vec, t2vec
    logical :: restart
    ...
    if (mixer%group_size == 0) then
        ! Secant errors are undefined. Hybrid scheme doesn't work.
        if (mixer%preferred_type == 1) then
            call msecant1_mix(mixer,parallel,iter,nrep,xin,xout,ierr)
        else  ! mixer%preferred_type == 2
            call msecant2_mix(mixer,parallel,iter,nrep,xin,xout,ierr)
        endif
        return
    endif
    ...
    if (m == 0) then
        ...
        ! where f0/f1 are the previous/current function values.
        ...
        if (restart) then
            ...
            if (mixer%en_stage /= 1) then
                mixer%n(:,res) = -mixer%param*mixer%dx1(:,res)
                do i=1,ngroup
                    ! Compute tmp = dx(:,(i-1)*sz+1:i*sz)'*mixer%dx1(:,res).
                    tmp(:) = zero
                    do j=1,sz
                        do k=1,dim
                            tmp(j) = tmp(j) + mixer%dx(k,(i-1)*sz+j)*mixer%dx1(k,res)
                        enddo
                    enddo
                    call psum(tmp, sz, parallel%group_size, parallel%group_comm)
            ...
            if (mixer%en_stage==1 .and. res==sz) then
                ...
                ! Determine Type-I or Type-II update on the approximate inverse Jacobian.
                if (mixer%en_stage/=1 .and. ngroup>0) then
                    ! The matrix norm used here is the Frobenius norm.
                    ! Compute t1 = norm(df(:,m-res+1:m)'*df2(:,sz-res+1:sz))^2.
                    t1 = 0.0
                    do i=1,res
                        do j=1,res
                            t0 = dot_product(mixer%df(:,m-res+i),mixer%df2(:,sz-res+j))
                            t0vec = t0
                            call psum(t0vec, 1, parallel%group_size, parallel%group_comm)
                            t0 = t0vec(1)
                            t1 = t1 + t0*t0
                        enddo
                    enddo
                    ! Compute t2 = norm(N(:,1:res)'*df(:,m-res+1:m))^2.
                    t2 = 0.0
                    do i=1,res
                        do j=1,res
                            t0 = dot_product(mixer%n(:,i),mixer%df(:,m-res+j))
                            t0vec = t0
                            call psum(t0vec, 1, parallel%group_size, parallel%group_comm)
                            t0 = t0vec(1)
                            t2 = t2 + t0*t0
                    ...
                    enddo
                    lhs = t1*t2
                    ! Compute t1 = norm(df(:,m-res+1:m)'*df(:,m-res+1:m),'fro').
                    t1 = 0.0
                    do i=1,res
                        do j=1,res
                            t0 = dot_product(mixer%df(:,m-res+i),mixer%df(:,m-res+j))
                            t0vec = t0
                            call psum(t0vec, 1, parallel%group_size, parallel%group_comm)
                            t0 = t0vec(1)
                            t1 = t1 + t0*t0
                        enddo
                    enddo
                    ! Compute t2 = norm(dx1(:,1:res)'*dx2(:,sz-res+1:sz))
                    t2 = 0.0
                    do i=1,res
                        do j=1,res
                            t0 = dot_product(mixer%dx1(:,i),mixer%dx2(:,sz-res+j))
                            t0vec = t0
                            call psum(t0vec, 1, parallel%group_size, parallel%group_comm)
                            t0 = t0vec(1)
                            t2 = t2 + t0*t0
                    ...
                    if (lhs > rhs) then
                        ...
                        mixer%update_type = mixer%preferred_type
                    endif
                else if (ngroup == 0) then
                    mixer%update_type = mixer%preferred_type
                endif
                ! If en_stage==1 and group_size==0, then type was already determined
                ! in the previous (sub-)iteration, and does not need recomputed again.
                ...
                if (mixer%update_type == 1) then  ! Type-I update.
                    ! Compute T = N'*df(:,m-res+1:m).
                    ...
                    if (info /= 0) then
                        ...
                        write(9,*) ' subroutine msecant3, info = ', info
                        ierr = 1
                    else
                        ! Compute T = N*Tinv'.
                        deallocate (mixer%t)
                        ...
                        if (res == sz) then
                            mixer%dx2 = mixer%dx1
                            mixer%df2 = mixer%df(:,m-res+1:m)
                            mixer%df(:,m-sz+1:m) = mixer%t(:,:)
                            ! df(:,m-sz+1:m) is now V(:,m-sz+1:m) in the note.
                        endif
                    endif
                else  ! mixer%update_type == 2, Type-II update.
                    ! Compute T = df(:,m-res+1:m)'*df(:,m-res+1:m).
                    ! Note that m = ngroup*sz + res.
                    do j=1,res
                        do i=j,res
                            do k=1,dim
                                mixer%t(i,j) = mixer%t(i,j) + mixer%df(k,ngroup*sz+i)*mixer%df(k,ngroup*sz+j)
                            enddo
                            mixer%t(j,i) = mixer%t(i,j)
                        enddo
                    enddo
                    call psum(mixer%t, res*res, parallel%group_size, parallel%group_comm)
                    call qinv(res, mixer%t, mixer%tinv, info)
                    if (info /= 0) then
                        ...
                        do i=1,dim
                            do j=1,res
                                do k=1,res
                                    mixer%t(i,j) = mixer%t(i,j) + mixer%df(i,m-res+k)*mixer%tinv(k,j)
                                enddo
                            enddo
                        enddo
                        ! Compute tmp = T'*f.

...
subroutine ewald_slab_pt(clust,pbc,eewald,ipr,zv,zvpt)
  ...
  ! Vector of Electric charge for each chemical element
  real(dp), intent(in) :: zv(clust%type_num)
  real(dp), intent(in) :: zvpt(clust%npttyp)
  ...
  ! Charged-sheet energy
  real(dp) :: enucsh
  ! merged zatom/zpt array
  real(dp) :: zatpt(pbc%atom_num*pbc%type_num+clust%nptchrg*clust%npttyp)

  ! Vector quantities:
  real(dp) :: &
       ratom(3, pbc%atom_num*pbc%type_num+clust%nptchrg*clust%npttyp), &   ! ratom(ia) = coordinates for atom no. ia
       zz(pbc%atom_num*pbc%type_num+clust%nptchrg*clust%npttyp)            ! zz(ia) = the electric charge on atom ia
  ...
  ! Counters:
  integer :: i, j, ia, ja, ig, natom, natom2, ii, jj, n
  ...
  real(dp), dimension(3,pbc%atom_num+clust%nptchrg) :: &
       fsumr, &          ! force sum over real-space
       fsumg, &          ! force sum over reciprocal space
       fsumm, &          ! general force sum over ia and ja
       tau               ! total force in latt vec coordinates
  ...
  allocate (pbc%ratpt (3,clust%nptchrg,clust%npttyp))
  natom2 = 0
  do i = 1, clust%npttyp
     do j = 1, clust%nptt(i)
        natom2 = natom2 + 1

        tmpvec(1)=clust%xpt(natom2)
        tmpvec(2)=clust%ypt(natom2)
        tmpvec(3)=clust%zpt(natom2)

        ! Multiplying by the bvec matrix to find
        ! the coordinates relative to lattice vec.
        ! Since (u,v,w) = (x,y,z) * A^-1 = (x,y,z) * B' /twopi
        call matvec3('T',pbc%bvec,tmpvec,pbc%ratpt(1:3,j,i))
     enddo
  enddo
  ...
  do i = 1, clust%npttyp
     do j = 1, clust%nptt(i)
        natom = natom + 1
        zz(natom) = zvpt(i)
        ratom(1:3,natom) = pbc%ratpt(1:3,j,i)*twopio
        write(9,*) 'ratom(',natom,')=',ratom(1:3,natom)
        do n = 1, 3
           if (ratom(n,natom) >= one) &
                ratom(n,natom) = modulo(ratom(n,natom) + eight,one)
  ...
  do i = 1, clust%type_num
     do j = 1, clust%natmi(i)
        natom2 = natom2 + 1
        zatpt(natom2) = clust%zatm(natom2)
     enddo
  enddo
  ...
  do ja = 1, natom
     sum_fact = zz(ja)
     do ia = 1, natom
        rrz = abs(zatpt(ja) - zatpt(ia))
        if(rrz>0) then
           rrz_sign = (zatpt(ja) - zatpt(ia))/rrz
        else
           rrz_sign = 1
        endif
        zdiff_eta = rrz*eta

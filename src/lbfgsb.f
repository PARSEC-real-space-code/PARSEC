...
c ----------------- the beginning of the loop --------------------------
 ...
 222  continue
      if (iprint .ge. 99) write (9,1001) iter + 1
      iword = -1
c
...
c                            skip the L-BFGS update.
         nskip = nskip + 1
         updatd = .false.
         if (iprint .ge. 1) write (9,1004) dr, ddum
         goto 888
      endif 
...
c                   this variable is always fixed
  ...
  20  continue
     ...
     +   'The initial X is infeasible.  Restart with its projection.'
         if (.not. cnstnd)
     +      write (9,*) 'This problem is unconstrained.'
      endif

      if (iprint .gt. 0) write (9,1001) nbdd
...
c       the derivative f1 and the vector p = W'd (for theta = 1).
      ...
      if (sbgnrm .le. zero) then
         if (iprint .ge. 0) write (9,*) 'Subgnorm = 0.  GCP = X.'
...
c                  is a zero vector, return with the initial xcp as GCP.
         if (iprint .gt. 100) write (9,1010) (xcp(i), i = 1, n)
         return
      endif    
...
c     Initialize derivative f2.
      ...
      nseg = 1
      if (iprint .ge. 99) 
     +   write (9,*) 'There are ',nbreak,'  breakpoints '
...
c       reset the corresponding component of d to zero.
      ...
      if (iprint .ge. 100) write (9,*) 'Variable  ',ibp,'  is fixed.'
...
c------------------- the end of the loop -------------------------------
 ...
 888  continue
      if (iprint .ge. 99) then
         write (9,*)
         write (9,*) 'GCP found in this segment'
         write (9,4010) nseg,f1,f2
         write (9,6010) dtm
      endif 
...
c            write(9,*) ' k  = index(i) ', k
c            write(9,*) ' index = ', i

            if (iwhere(k) .gt. 0) then
               ileave = ileave - 1
               indx2(ileave) = k
               if (iprint .ge. 100) write (9,*)
...
c                               Line search is impossible.
            write(9,*)' ascent direction in projection gd = ', gd
            info = -4
            return
         endif
      endif
      ...
      if (iprint .ge. 0) then
         ...
         if (iprint .ge. 1) then
            ...
            if (iprint .gt. 100) then
               write (9,1004) 'L =',(l(i),i = 1,n)
               write (9,1004) 'X0 =',(x(i),i = 1,n)
               write (9,1004) 'U =',(u(i),i = 1,n)
            endif 
         endif
      endif 
...
c                             the truncated Newton step has been used.
      ...
      if (iprint .ge. 99) then
         write (9,*) 'LINE SEARCH',iback,' times; norm of step = ',xstep
         write (9,2001) iter,f,sbgnrm
         if (iprint .gt. 100) then      
            write (9,1004) 'X =',(x(i), i = 1, n)
            write (9,1004) 'G =',(g(i), i = 1, n)
         endif
      else if (iprint .gt. 0) then 
         imod = mod(iter,iprint)
         if (imod .eq. 0) write (9,2001) iter,f,sbgnrm
      endif
...
c     ************
      ...
      if (iprint .ge. 0) then
         write (9,3003)
         write (9,3004)
         write(9,3005) n,iter,nfgv,nintol,nskip,nact,sbgnrm,f
         if (iprint .ge. 100) then
            write (9,1004) 'X =',(x(i),i = 1,n)
         endif  
         if (iprint .ge. 1) write (9,*) ' F =',f
      endif 
 999  continue
      if (iprint .ge. 0) then
         write (9,3009) task
         if (info .ne. 0) then
            if (info .eq. -1) write (9,9011)
            if (info .eq. -2) write (9,9012)
            if (info .eq. -3) write (9,9013)
            if (info .eq. -4) write (9,9014)
            if (info .eq. -5) write (9,9015)
            if (info .eq. -6) write (9,*)' Input nbd(',k,') is invalid.'
            if (info .eq. -7) 
     +      write (9,*)' l(',k,') > u(',k,').  No feasible solution.'
            if (info .eq. -8) write (9,9018)
            if (info .eq. -9) write (9,9019)
         endif
         if (iprint .ge. 1) write (9,3007) cachyt,sbtime,lnscht
         write (9,3008) time
         if (iprint .ge. 1) then
 ...
 3008 format (/,' Tot. User time',1p,e10.3,' seconds.',/)
 3009 format (/,a60)
 9011 format (/,
...
c
 ...
 55   continue
      if ( dd_p .gt.zero ) then
         ...
         write(9,*) ' Positive dir derivative in projection '
         write(9,*) ' Using the backtracking step '
      else
         go to 911
      endif
c

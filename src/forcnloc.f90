  ...
  if (parallel%iammaster) then
     ...
     if (ipr >= 1) then
        ...
        do j = 1,clust%atom_num
           write(7,40)  "F",j,clust%xatm(j),clust%yatm(j),clust%zatm(j), &
                clust%force(:,j)
        enddo
        write(7,*)
        write(7,*)
...
! why only the master inializes this?
        ...
        do lp = 1, nlp
           ...
           if (lp == local) cycle
           write(9,*) ' I received L = ', l, ' for atom #', ja, &
                ' with mnloc = ', mnloc
           do locpnt = 1, mnloc
              do i = 1, 2*lp - 1
           ...
           do isp = 1, elec_st%nspin
              ...
              if (elec_st%cplx) then
                 ...
                 call zpsum(ztvywd(1:nstate,0,1,1,l*l + 1,isp),4*(2*l + 1) &
                      *nstate*nrep*kpnum*2,parallel%procs_num,comm)
        ...
        if (parallel%iammaster) then
           ...
           if (ipr >= 1) then
              write(7,40) "F", ja, clust%xatm(ja), clust%yatm(ja), &
                   clust%zatm(ja), fnloc
           endif
  ...
  if (parallel%iammaster) then
     ...
     write(7,41)
     do  j = 1,clust%atom_num
        write(7,40) "Q",j,clust%xatm(j),clust%yatm(j),clust%zatm(j), &
             clust%force(:,j) - fatom
     enddo
     ...
     if (pot%fex_is == .false.) then 
       ...
       do  j = 1,clust%atom_num
        clust%force(:,j) = clust%force(:,j) 
       enddo
       write(7,*) "Net forces not included in FDET!"
     endif
     write(7,*) "Check!"
  ...
  ! format statements
40 format(A3,i4,2x,3(f11.6,1x),2x,3(f11.6,1x))
